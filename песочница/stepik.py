...
class Testpaper:
    ...
    def __mod__(self, other: List):
        t_answer = 0
        for el1, el2 in zip(self.answer, other):
            if el1 == el2:
                t_answer+=1
        return round(t_answer/len(self.answer)*100)
...
class Student:
    def __init__(self):
        self.tests = {}
    def take_test(self, test: Testpaper, answer: List[str]):
        res = test%answer
        self.tests[test.subject] = ' '.join(
            ('Failed!' if res < int(test.true[:-1]) else 'Passed!',
             f'({res}%)')
        )
